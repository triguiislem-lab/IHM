...
import { MdCheckCircle, MdWarning, MdLock, MdPlayCircleFilled } from 'react-icons/md';
...
const ModuleProgressCard = ({ moduleId, courseId, moduleData, index }) => {
  ...
  return (
    <div className={`bg-white p-4 rounded-lg shadow-md mb-4 ${!isAccessible ? 'opacity-70' : ''}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <h3 className="font-medium text-lg">{moduleData.title || `Module ${index + 1}`}</h3>
            {progress && (
              <span className={`text-sm font-medium px-2 py-1 rounded-full ${
                progress.completed ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
              }`}>
                {progress.score ? `${progress.score.toFixed(0)}%` : 'En cours'}
              </span>
            )}
          </div>
          
          <p className="text-gray-600 text-sm mb-3">{moduleData.description || 'Aucune description disponible'}</p>
          ...
          {progress && (
            <div className="mb-3">
              ...
              <div className="flex justify-between mt-1">
                <span className="text-xs text-gray-500">Progression: {progress.progress || 0}%</span>
                {progress.lastUpdated && (
                  <span className="text-xs text-gray-500">
                    Dernière activité: {new Date(progress.lastUpdated).toLocaleDateString()}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex justify-end">
            {isAccessible ? (
              ...
              >
                {progress && progress.completed ? 'Revoir le module' : 'Continuer'}
              </Link>
            ) : (

...
const CourseForm = () => {
  ...
  const loadInitialData = useCallback(async () => {
    if (authLoading || !user || user.normalizedRole !== 'instructor') {
      if (!authLoading) setPermissionError("Accès réservé aux formateurs.");
      ...
      if (isEditMode) {
        ...
        if (course) {
          if (course.instructorId !== user.uid) {
            setPermissionError("Vous ne pouvez modifier que vos propres formations.");
            setLoadingData(false);
            return;
          }
          setCourseData({
          ...
          if (course.specialiteId) {
            const filtered = disciplinesData.filter(d => d.specialiteId === course.specialiteId);
        ...
        setCourseData(prev => ({ ...prev, instructorId: user.uid }));
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (courseData.specialiteId) {
      ...
      setFilteredDisciplines(filtered);
      if (courseData.disciplineId && !filtered.some(d => d.id === courseData.disciplineId)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode && courseData.instructorId !== user.uid) {
      setError("Impossible de modifier une formation qui ne vous appartient pas.");
      return;
    }
    ...
    try {
      if (!courseData.title || courseData.title.trim().length < 5) throw new Error("Titre invalide (min 5 caractères).");
      if (!courseData.description || courseData.description.trim().length < 10) throw new Error("Description invalide (min 10 caractères).");
      if (!courseData.duration) throw new Error("Durée manquante.");
      if (courseData.price < 0) throw new Error("Prix invalide.");
      if (!courseData.specialiteId) throw new Error("Spécialité manquante.");
      if (!courseData.disciplineId) throw new Error("Discipline manquante.");
      ...
      if (!isEditMode) {
        navigate(`/instructor/course-form/${courseId}`);
      } else {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <Link to="/instructor/courses" className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-4">
        <MdArrowBack className="mr-2" /> Retour à mes formations
      </Link>
      <h1 className="text-2xl font-bold mb-6">
        {isEditMode ? "Modifier Ma Formation" : "Créer une Nouvelle Formation"}
      </h1>
      ...
      <div className="mb-6 border-b border-gray-200">
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${ activeTab === 'info' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }`}
          > Informations Générales </button>
          <button
            onClick={() => setActiveTab('modules')}
            disabled={!isEditMode}
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${ activeTab === 'modules' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' } ${!isEditMode ? 'cursor-not-allowed opacity-50' : ''}`}
          > Modules & Contenu </button>
        </nav>
      </div>
